package com.birdsoaring.sources;
...
public class KuduInputFormat implements InputFormat<RowSerializable, KuduInputSplit> {
    ...
    public void configure(Configuration parameters) {
        logger.info("2. CONFIGURE");
        logger.info("Initializing KUDU Configuration...");
    ...
    public void open(KuduInputSplit split) throws IOException {
        ...
        logger.info("SPLIT " + split.getSplitNumber() + " PASANDO POR 5. OPEN");
        if (table == null) {
            throw new IOException("The Kudu table has not been opened!");
        }

        logger.info("Opening split...");
        ...
        try {
            logger.info("SPLIT NUMBER " + split.getSplitNumber());
    ...
    public RowSerializable nextRecord(RowSerializable reuse) throws IOException {
        ...
        try {
            ...
            logger.warn("Error after scan of " + scannedRows + " rows. Retry with a new scanner...", e);
        }
        return null;
    }
    ...
    public void close() throws IOException {
        logger.info("Closing split (scanned {} rows)" + scannedRows);
    ...
    public KuduInputSplit[] createInputSplits(final int minNumSplits) {
        ...
        logger.info("Created: " + splits.size() + " splits");
